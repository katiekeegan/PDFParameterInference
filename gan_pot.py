...
def train_loop(
    ...
    for _ in range(args.S_steps):
        ...
        if torch.isnan(data).any():
            ...
            return
        gen_prior_params = Generator(z)
        breakpoint()
        gen_params = torch.tensor([torch.normal(mean=gen_prior_params[i,0], std=gen_prior_params[i,1])for i in gen_prior_params.size(0)])
        ...
        S_W = SurrogatePhysics(gen_params)
