...
def train_loop(
    ...
    stop_time = time.time()
    other_time += stop_time - start_time
    for _ in range(args.D_steps):
        # Record start time
        feedforward_G_start = time.time()
        ...
        if torch.isnan(gen_data).any():
            ...
            return
        feedforward_G_end = time.time()
        feedforward_G_time += feedforward_G_end - feedforward_G_start
        D_start = time.time()
        ...
        D_loss = floss #+ 0.01*torch.norm(torch.log(torch.std(data_p)+1e-8)-torch.log(torch.std(gen_data_p)+1e-8))
        ...
        for _ in range(args.S_steps):
            ...
            S_time += S_time_end - S_time_start
    ...
    print(f"Feedforward G Time {feedforward_G_time}")
    print(f"D time: {D_time}")
    print(f"S time: {S_time}")
    print(f"G time: {G_time}")
