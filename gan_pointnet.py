...
def train_loop(
    ...
    for _ in range(args.D_steps):
        ...
        print(data.size())
        # breakpoint()
        toutput = Discriminator_p(torch.squeeze(data_p).unsqueeze(0).unsqueeze(0))
        tlabels = torch.ones(1,1).to(device)
        ...
        flabels = torch.zeros(1,1).to(device)

        foutput = Discriminator_p(torch.squeeze(gen_data_p).unsqueeze(0).unsqueeze(0))
        ...
        toutput = Discriminator_n(torch.squeeze(data_n).unsqueeze(0).unsqueeze(0))
        tloss = criterion(toutput.squeeze(), tlabels.squeeze())
        ...
        foutput = Discriminator_n(torch.squeeze(gen_data_n).unsqueeze(0).unsqueeze(0))
        if torch.isnan(foutput).any() or torch.isinf(foutput).any():
            continue
        ...
        for _ in range(args.S_steps):
            ...
            gen_data_p = gen_data[...,0]
            gen_data_n = gen_data[...,1]
            foutput_p = Discriminator_p(torch.squeeze(gen_data_p).unsqueeze(0).unsqueeze(0))
            foutput_n = Discriminator_n(torch.squeeze(gen_data_n).unsqueeze(0).unsqueeze(0))
            # breakpoint()
...
def main():
    ...
    parser.add_argument(
        "--lr", type=float, default=0.00001, help="learning rate (default: 0.1)"
    )
    parser.add_argument(
    ...
    Discriminator_p = SimplePointNet().to(device)
    Discriminator_n = SimplePointNet().to(device)
