...
def train_loop(
    ...
    for _ in range(args.D_steps):
        ...
        toutput = Discriminator(torch.squeeze(data).unsqueeze(-1))
        ...
        foutput = Discriminator(torch.squeeze(gen_data).unsqueeze(-1))
        ...
        D_loss = floss # + torch.norm(gen_data.mean()-data.mean())
        D_losses.append(D_loss.detach().cpu().item())
        D_loss.backward()
        if torch.isnan(D_loss).any():
            print(f"NaN detected in D_loss")
    ...
    with open('results/poisson/' + filename + "training_metrics_3.json", "w") as f:
        json.dump(metrics, f)
    ...
    if args.plot == True:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("results/poisson/" + filename + "scatterplot_3.png")
        plt.draw()
        plt.pause(0.0001)
        plt.close()
        fig2 = plt.figure(2)
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("results/poisson/" + filename + "G_losses_3.png")
        plt.draw()
        plt.pause(0.0001)
        plt.close()
        fig3 = plt.figure(3)
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("results/poisson/" + filename + "D_losses_3.png")
        plt.draw()
        plt.pause(0.0001)
        plt.close()
        fig4 = plt.figure(4)
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("results/poisson/" + filename + "S_losses_3.png")
        plt.draw()
        plt.pause(0.0001)
        plt.close()
        fig5 = plt.figure(5)
        ...
        plt.tight_layout()
        plt.savefig("results/poisson/" + filename + "parameters_3.png")
        plt.draw()
        plt.pause(0.0001)
        plt.close()
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=7,
        help="Lambda in Poisson distribution (default: 7)",
    )
    parser.add_argument(
        "--budget",
        type=int,
        default=160000,
        help="Simulation budget (default: 1024000)",
    )
    parser.add_argument(
        "--filename",
        type=str,
    ...
    tparams = torch.tensor([float(args.lmbd)])
    # # Set true parameters
    ...
    Generator = PoissonGenerator(args.noise_dim, param_dims).to(device)
    Discriminator = PoissonDiscriminator(input_dim = 1).to(device)
    SurrogatePhysics = PoissonSurrogate(input_dim=param_dims).to(device)
    ...
    D_Optimizer = optim.Adam(Discriminator.parameters(), lr=0.1*args.lr)
    S_Optimizer = optim.Adam(SurrogatePhysics.parameters(), lr=0.1*args.lr)

    print(f"Number of trainable parameters in G: {sum(p.numel() for p in Generator.parameters() if p.requires_grad)}")
    print(f"Number of trainable parameters in D: {sum(p.numel() for p in Discriminator.parameters() if p.requires_grad)}")
    ...
    for epoch in range(args.epochs):
        ...
        for data in data_loader:
            ...
            budget += args.batch_size*args.D_steps + args.batch_size*args.S_steps
            print("Budget")
            print(budget)
    ...
    torch.save(Generator.state_dict(), "results/poisson/" + filename + "model_3.pt")
